...
export const getHydratedDomainObjectMetadatasFromFiles = (
  ...
  const domainObjectMetadatas = files.flatMap((file) => {
  ...
  const enumMetadatas = files.flatMap((file) => {
  ...
  const referencedAliasNames: string[] = domainObjectMetadatas
    ...
    .filter(isPresent);
  const aliasMetadatas = files.flatMap((file) => {
    const aliasDeclarationsAll = file.statements.filter(isTypeAliasDeclaration);
